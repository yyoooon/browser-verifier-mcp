...
import {
  extractSemanticState,
  type SemanticState,
} from "../semantic/extractSemanticState.js";
...
export async function runVerify(
  ...
  const needsLoadedRetry = checks.find(
    (c): c is Extract<VerifyCheck, { type: "loaded" }> => c.type === "loaded",
  );
  if (
    needsLoadedRetry &&
    state.loading &&
  ...
  const domResultsByIdx = new Map<number, DomQueryResult>();
  if (domQueries.length > 0) {
    const raw = await runDomQueries(page, domQueries);
    for (const r of raw) domResultsByIdx.set(r.idx, r);
  }
...
async function runDomQueries(
  ...
  return page.evaluate((qs) => {
    return qs.map((q) => {
      ...
      if (!el) return { idx: q.idx, found: false };
...
function runDomCheck(
  check: VerifyCheck,
  result: DomQueryResult | undefined,
): CheckResult {
  if (
...
function runStateCheck(check: VerifyCheck, state: SemanticState): CheckResult {
  switch (check.type) {
    case "primary_cta": {
      if (!state.primaryCTA) {
        return {
    ...
    case "loaded": {
      if (!state.loading) return { type: "loaded", ok: true };
      return {
        type: "loaded",
        ok: false,
