...
export const fillDefinition: Tool = {
  name: "browser_fill",
  description:
    "Fill a text input or textarea, safe for React controlled components. " +
...
export async function fillHandler(args: { selector: string; value: string }) {
  if (!args.selector) return fail("selector is required.");
  if (args.value === undefined || args.value === null)
    return fail("value is required (use empty string to clear).");
  const r = await fillReactInput(args.selector, args.value);
  if (!r.ok) return fail(r.error ?? "fill failed", { elapsedMs: r.elapsedMs });
  return ok({
    ok: true,
  ...
  inputSchema: {
    ...
    properties: {
      ...
      text: {
        type: "string",
        description:
          "Visible text to match (exact preferred, partial fallback). Searches button/a/[role=button|link|tab|menuitem]/input[type=submit|button]/label/[data-slot].",
      },
    },
  },
};
...
export const pressKeyDefinition: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      key: {
        type: "string",
        description:
          "Key name (W3C): 'Enter', 'Escape', 'Tab', 'ArrowDown', 'Backspace', etc.",
      },
    },
    required: ["key"],
  },
...
export const selectOptionDefinition: Tool = {
  name: "browser_select_option",
  description:
    "Open a custom dropdown (Radix Select / Headless UI Listbox / shadcn Select / any [role=combobox] trigger that opens a [role=listbox] with [role=option] children) and pick an option by text. " +
  ...
  inputSchema: {
    ...
    properties: {
      triggerSelector: {
        type: "string",
        description:
          "CSS selector for the trigger button (often button[role='combobox']).",
      },
      triggerText: {
        type: "string",
        description:
          "Visible text on the trigger (alternative to triggerSelector).",
      },
      optionText: {
        type: "string",
...
export async function selectOptionHandler(args: {
  ...
  const t0 = Date.now();
  if (!args.optionText) return fail("optionText is required.");
  ...
  try {
    ...
    if (!pickR.ok) {
      return fail(
        `failed to pick option "${args.optionText}": ${pickR.error ?? "unknown"}`,
        { elapsedMs: Date.now() - t0 },
      );
    }
...
export const navigateDefinition: Tool = {
  ...
  description:
    "Navigate the attached page via page.goto() — clean alternative to location.href in browser_eval (which destroys the eval execution context mid-call). " +
