...
export const definition: Tool = {
  name: "browser_setup",
  description:
    "Attach to a Chrome page target via CDP. Two ports involved:\n" +
  ...
  inputSchema: {
    ...
    properties: {
      ...
      cdpPort: {
        type: "number",
        description:
          "Chrome remote-debugging port (default 9223). Use when Chrome was launched with a non-default --remote-debugging-port. For Docker/WSL setups with a non-localhost host, set BROWSER_VERIFIER_CDP_URL env instead.",
      },
    },
  },
};

export async function handler(args: { port?: number; cdpPort?: number }) {
  const t0 = Date.now();
