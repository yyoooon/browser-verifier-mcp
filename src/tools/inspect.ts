...
export async function handler(args: { targets?: unknown }) {
  const { targets } = args;
  if (
    ...
    return fail(
      `browser_inspect requires 'targets' to be an object keyed by label, ` +
  ...
  try {
    ...
    const result = await runInspect(state.page, targets as InspectInput);
    return ok(result);
  } catch (e) {
