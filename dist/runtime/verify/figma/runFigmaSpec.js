...
import { installTransitionGuard, removeTransitionGuard, } from "./transitionGuard.js";
...
export async function runFigmaSpec(page, spec) {
    const results = [];
...
async function readRootCssVariables(page, names) {
...
async function runTarget(page, target) {
    ...
    if (expected.length === 0 && classNames.length === 0)
        return [];
    try {
    ...
    try {
        measured = await measure(page, target.selector, measureProps, classNames.length > 0);
    }
    finally {
        try {
    ...
    if (swatches.length > 0) {
        ...
        swatches.forEach((s, i) => {
            ...
            const unresolvedHint = sw.value === sw.defaultValue
...
function flattenExpected(target) {
    const out = [];
    const t = target.typography ?? {};
    if (t.fontSize)
        out.push({ prop: "fontSize", value: t.fontSize });
    if (t.fontWeight)
        out.push({ prop: "fontWeight", value: t.fontWeight });
    if (t.lineHeight)
        out.push({ prop: "lineHeight", value: t.lineHeight });
    if (t.letterSpacing)
        out.push({ prop: "letterSpacing", value: t.letterSpacing });
    if (t.fontFamily)
        out.push({ prop: "fontFamily", value: t.fontFamily });
...
async function measure(page, selector, props, includeClassList) {
    return page.evaluate(({ sel, ps, withClasses }) => {
        const el = document.querySelector(sel);
        if (!el)
            return { found: false, values: {}, classList: [] };
...
async function measureSwatches(page, selector, swatches) {
...
//# sourceMappingURL=runFigmaSpec.js.map