export async function extractSemanticState(page) {
    const t0 = Date.now();
...
function extractInPage() {
    ...
    if (progressEl && visible(progressEl))
        loadingHints.push("role=progressbar");
    const skeletons = Array.from(document.querySelectorAll("[data-slot~=skeleton], [class*=skeleton i], [class*=Skeleton]")).filter(visible);
    if (skeletons.length > 0) {
        loadingHints.push(`skeleton×${Math.min(skeletons.length, 99)}`);
    }
    const spinners = Array.from(document.querySelectorAll("[class*=spinner i], [class*=Spinner], [class*=loading i]:not([class*=heading i])")).filter(visible);
    ...
    document
        ...
        if (dataState && dataState !== "open")
            return;
        if (visible(el))
            dialogCandidates.push(el);
    });
    if (dialogCandidates.length > 0) {
        const top = dialogCandidates[dialogCandidates.length - 1];
        const titleEl = top.querySelector("[role=heading], h1, h2, h3, [data-slot=dialog-title]") ?? top.querySelector("[id*=title i]");
        modal = {
    ...
    const isInLandmark = (el, tags, roles) => {
        let p = el;
        while (p && p !== document.body) {
            if (tags.includes(p.tagName))
                return true;
    ...
    const ctaScope = modal
        ? dialogCandidates[dialogCandidates.length - 1]
        : document.body;
    let primaryCTA = null;
    const ctaCandidates = Array.from(ctaScope.querySelectorAll("button[type=submit], input[type=submit], button, [role=button]"))
        .filter((el) => visible(el))
        .filter((el) => modal ||
        !isInLandmark(el, ["NAV", "ASIDE", "FOOTER"], ["navigation", "complementary", "contentinfo"]))
        .filter((el) => {
        ...
        const looksLikeContentCard = he.querySelectorAll("*").length > 6 ||
            he.querySelector("h1, h2, h3, h4, h5, h6, [role=heading], p") !== null;
        const prominent = area >= MIN_PROMINENT_AREA && shortLabel && !looksLikeContentCard;
        let score = 0;
        if (isSubmit)
            score += 60;
        if (he.tagName === "BUTTON")
            score += 5;
        ...
        if (primaryMatch)
            ...
            score += 8;
        if (dismissMatch)
            score -= 30; // demote close/cancel below the real action
        if (!enabled)
            score -= 15; // prefer an enabled action when ranking
    ...
    const errorElements = [];
    document
        .querySelectorAll("[role=alert], [aria-live=assertive], [aria-invalid=true], [data-state=error], [data-slot=form-message]")
        .forEach((el) => {
    ...
    for (const el of errorElements) {
        ...
        seen.add(t);
        errors.push(t);
        if (errors.length >= LIST_CAP)
            break;
    }
    // ---- inputs ----
    const inputCount = Array.from(document.querySelectorAll("input, textarea, select")).filter(visible).length;
    // ---- focused ----
...
//# sourceMappingURL=extractSemanticState.js.map