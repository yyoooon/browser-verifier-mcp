...
import { getSourcePath, getTasks, setTasks, } from "../runtime/tasks/registry.js";
...
export async function loadHandler(args) {
    try {
        const result = loadTasksFromFile(args.path);
...
export const listDefinition = {
    ...
    description: "List currently loaded tasks with their description, declared args, and step count. Use this to discover what tasks are available before calling browser_run_task.",
...
export const runDefinition = {
    ...
    description: "Execute task steps deterministically (bail on first failure). Two modes:\n" +
    ...
    inputSchema: {
        ...
        properties: {
            ...
            steps: {
                type: "array",
                description: "Inline step list (same shape as task steps). Provide name OR steps.",
                items: {
                    type: "object",
            ...
            args: {
                type: "object",
                description: "Key/value map. Values get coerced to strings during {{argName}} substitution.",
                additionalProperties: true,
            },
        },
    },
