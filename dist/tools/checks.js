...
import { getConsole, clearConsole, getNetwork, clearNetwork, flushConsole, } from "../cdp/buffers.js";
...
export const consoleDefinition = {
    name: "browser_check_console",
    description: "Drain the in-server console buffer accumulated since attach (or last clear). Filters known dev-mode noise (CareHubBridge / HMR / Fast Refresh). Use after interactions to detect JS errors. Pass clear:true to flush after read.",
    inputSchema: {
        type: "object",
        properties: {
            level: {
                ...
                description: 'Filter by level (default "all", "error" returns errors+exceptions).',
            },
...
export const networkDefinition = {
    ...
    description: "Drain the in-server network buffer accumulated since attach. Returns failed (4xx/5xx/error) requests by default. Pass status to filter, clear:true to flush.",
    inputSchema: {
        type: "object",
        properties: {
            status: {
...
export const urlDefinition = {
    name: "browser_get_url",
    description: "Get the current location.href of the attached page.",
    inputSchema: { type: "object", properties: {} },
};
export const visibilityDefinition = {
    name: "browser_is_visible",
    description: "Check if a CSS selector matches an element that is in the DOM and has non-zero client rects (not display:none, not visibility:hidden).",
    inputSchema: {
        type: "object",
...
export async function consoleHandler(args) {
    ...
    const filtered = all.filter((e) => {
        if (!args.includeNoise && isNoise(e.text))
            return false;
...
export async function visibilityHandler(args) {
    const script = `
    (() => {
