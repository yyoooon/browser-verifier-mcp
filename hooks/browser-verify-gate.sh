...
#
# Opt-in: $HOME/.browser-verifier-auto 파일이 있을 때만 동작.
# Plugin 사용자는 /browser-verifier:enable-auto 슬래시 명령으로 토글.
# Manual 사용자는 직접 `touch $HOME/.browser-verifier-auto` / `rm $HOME/.browser-verifier-auto`.

set -euo pipefail

# 명시적 opt-in 파일이 없으면 아무 일도 안 함
[ -f "$HOME/.browser-verifier-auto" ] || exit 0
...
#    CUSTOMIZE: 아래 case 패턴을 소비 프로젝트의 검증 대상 경로로 교체.
changed=$({
...
cat >&2 <<'EOF'
[auto-verify] 코드 변경이 감지됐습니다. Skill 툴로 `browser-verifier:verify` 스킬을 invoke해서 검증 사이클을 시작하세요.

사이클 종료(PASS / SKIP / ESCALATION) 시 `browser_sentinel_save`로 ".claude/.last-verified-hash"에 현재 diff hash를 기록해야 다음 Stop에서 무한 루프가 안 납니다.
EOF
exit 2
