# browser-verifier-mcp 총정리 — 처음 보는 동료에게 소개하고, 어떤 질문에도 답하기
...
v0.4.0부터 **검증 전용(verification-only)** 으로 운영한다. 직접 조작 도구(click/fill/navigate)는 제거했고, 조작은 페어 도구 `agent-browser`에 위임한다 (이유는 [Q&A 8-4](#8-4-왜-조작-기능을-제거했나) 참고).
...
| 문서 | 내용 |
|---|---|
| [`쉽게-이해하기.md`](./쉽게-이해하기.md) | 가장 쉬운 비유(채점) 중심 입문 |
| [`이-MCP-소개.md`](./이-MCP-소개.md) | 왜/누구/무엇 전체 개요 |
| [`browser_verify-내부동작.md`](./browser_verify-내부동작.md) | verify 4단계를 코드와 함께 |
| [`concepts.md`](./concepts.md) | LLM·CDP·MCP 기초 개념 + 용어 사전 |
| [`refactor-phase-1~9.md`](./refactor-phase-1.md) | raw CDP → Playwright → task → plugin 진화 기록 |
